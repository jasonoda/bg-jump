...
export class Scene {
    ...
    buildScene() {
        ...
        // Breadcrumb tracking
        this.levelScore = 0; // Score earned in current level
        this.levelMatchPoints = []; // Points from each match in current level
        this.levelStartTime = performance.now(); // Start time of current level
        ...
        // Read desired play area max width from CSS once
        try {
            const rootStyle = getComputedStyle(document.documentElement);
            const cssVar = rootStyle.getPropertyValue('--play-area-width') || '480px';
            const parsed = parseInt(cssVar.toString().trim().replace('px',''));
            this.playAreaMaxWidthPx = Number.isFinite(parsed) ? parsed : 480;
            // Ensure the runtime var matches the intended width
            document.documentElement.style.setProperty('--play-area-width', `${this.playAreaMaxWidthPx}px`);
        } catch(e) {
            this.playAreaMaxWidthPx = 480;
            document.documentElement.style.setProperty('--play-area-width', `480px`);
        }
    ...
    createCardGrid(x, y) {
        ...
        this.cardContainer.className = 'card-grid';
    ...
    updatePlayAreaWidthVar() {
        // Lock blockers to the configured max play area width; do not shrink based on grid
        try {
            const widthPx = this.playAreaMaxWidthPx || 480;
            document.documentElement.style.setProperty('--play-area-width', `${widthPx}px`);
        } catch (e) {
            // no-op
        }
    }
    ...
    handleMatch() {
        ...
        this.levelScore += totalPoints; // Track level score
        this.levelMatchPoints.push(totalPoints); // Track individual match points
        
        console.log("=== MATCH SCORING DEBUG ===");
        console.log("Match points earned:", totalPoints);
        console.log("Level score so far:", this.levelScore);
        console.log("Total game score:", this.score);
        console.log("Level match points array:", this.levelMatchPoints);
        console.log("=============================");
    ...
    handleGameComplete() {
        ...
        console.log("Level score to be stored:", this.levelScore);
        console.log("Game scores array before push:", this.gameScores);
        
        this.gameScores.push(this.levelScore); // Store level score, not total score
        
        console.log("Game scores array after push:", this.gameScores);
        console.log("Total game score:", this.score);
        console.log("===============================");
        
        // Create breadcrumb for completed level
    ...
    startNextLevel() {
        ...
        console.log("=== LEVEL RESET DEBUG ===");
        console.log("Resetting level tracking for level", this.currentLevel + 1);
        console.log("Previous level score was:", this.levelScore);
        console.log("Previous level match points were:", this.levelMatchPoints);
        
        this.levelScore = 0;
        this.levelMatchPoints = [];
        this.levelStartTime = performance.now();
        
        console.log("Reset complete - level score:", this.levelScore, "match points:", this.levelMatchPoints);
        console.log("==========================");
    ...
    endGame() {
        ...
        // Add final incomplete level score to gameScores before validation
        console.log("=== END GAME DEBUG ===");
        console.log("Game ended - adding final level score:", this.levelScore);
        console.log("Game scores before adding final level:", this.gameScores);
        
        this.gameScores.push(this.levelScore); // Add final incomplete level score
        
        console.log("Game scores after adding final level:", this.gameScores);
        console.log("Total game score:", this.score);
        console.log("======================");
    ...
    breadCrumb(type){
        ...
        const breadCrumbPayload = {
            ...
            levelMatchPoints: this.levelMatchPoints,
